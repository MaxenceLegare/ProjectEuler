...
int main() {
    ...
    for(int block = 0; block <= n_block; block++) {
         ...
         if( block < longueur_min ){
          continue;
        }

         arr[block] += arr[block-1];

         for(int longueur_block = longueur_min ; longueur_block <= n_block; longueur_block++) {
            int n_restant = block - longueur_block - 1;
            if( n_restant >= 0 ){
             arr[block] += arr[n_restant];
           }
        }
    }
